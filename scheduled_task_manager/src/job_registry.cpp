...
void JobRegistry::initialize(const SchedulerConfig &config,
  ...
  // TODO: DB-driven migration — load from scheduled_jobs table, merge with config
  std::cout << "[JobRegistry] Registering all scheduled jobs...\n";

  // Helper lambda to create jobs concisely
  ...
  register_job(make_job("baseline_recalculation", "Baseline Recalculation",
  ...
  // RETRY JOBS (meta-jobs: don't themselves retry)
  register_job(make_job("failed_sync_retry", "Failed Sync Retry",
  ...
  register_job(make_job("notification_retry", "Notification Retry",
    JobCategory::RETRY, JobPriority::MEDIUM,
  ...
  // CLEANUP JOBS
  register_job(make_job("data_cleanup", "Data Cleanup",
  ...
  for (const auto &job : jobs_) {
    std::cout << "  [" << (job.enabled ? "ON " : "OFF") << "] " << job.name
              << " — every " << job.interval_seconds << "s, priority="
