...
void prepare_device_job_statements(storage_handoff::StorageWriter &writer) {
  // Check device last_seen_at staleness (returns minutes since last seen)
...
JobResult device_health_check(const nlohmann::json &config,
  ...
  // ── 1. Check device staleness (heartbeat) ──
  double stale_mins =
      writer.query_double("stm_device_staleness_minutes", device_id);
  if (stale_mins > 10.0) {
    std::cout << "[DeviceJobs] Device offline: last seen " << stale_mins
              << " minutes ago.\n";
    std::string severity = (stale_mins > 30.0) ? "high" : "medium";
    nlohmann::json payload = {{"minutes_since_seen", stale_mins}};
    ...
    writer.execute_prepared("stm_emit_notification",
        ...
        std::string("Device Offline"),
        "Device has not sent a heartbeat in " + std::to_string(static_cast<int>(stale_mins)) + " minutes.",
        severity,
        payload.dump(),
        "STM_NOTIF_OFFLINE_" + device_id);

    result.items_processed++;
    result.metadata["offline_minutes"] = stale_mins;
  }

  // ── 2. Check battery level ──
  double battery = writer.query_double("stm_device_battery_level", device_id);
  if (battery > 0.0 && battery < 20.0) {
    std::string severity = (battery < 5.0) ? "critical" : "medium";
    std::string event_type =
        (battery < 5.0) ? "critical_battery" : "low_battery";
    nlohmann::json payload = {{"battery_percent", battery}};
    writer.execute_prepared("stm_device_emit_event",
