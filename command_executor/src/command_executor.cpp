...
CommandExecutor::CommandExecutor(CommandQueue& queue, std::function<void(const Command&)> on_result_cb)
...
void CommandExecutor::run() {
    ...
    while (running_) {
        ...
        std::cout << "[CommandExecutor] Processing signal_id: " << cmd.signal_id << " (" << cmd.signal_type << ")\n";
        ...
        switch (cmd.signal_id) {
            ...
            case 88:
                ...
                break;
            default:
                std::cerr << "[CommandExecutor] Error: No handler registered for signal_id " << cmd.signal_id << "\n";
                res.success = false;
        ...
        // Perform the dummy database log
        SignalLogger::log(cmd);

        // Forward back to caller (the responder logic / main ZMQ callback)
        if (on_result_cb_) {
            on_result_cb_(cmd);
        }
    }
